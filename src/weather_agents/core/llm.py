"""LLM abstraction layer using LiteLLM with retry, fallback, and token tracking."""
...
# Rough cost per 1K tokens (input / output) for known models — USD
...
def estimate_cost(model: str, prompt_tokens: int, completion_tokens: int) -> float:
    ...
    costs = _MODEL_COST_ESTIMATES.get(model, (0.001, 0.002))  # fallback guess
...
class LLMClient:
    ...
    Features retry with exponential backoff, model fallback chains,
    ...
    def __init__(self, config: AppConfig, tool_registry: ToolRegistry) -> None:
        ...
        self._usage_stats: dict[str, dict] = {}  # agent_name -> stats
    ...
    def _track_usage(self, agent_name: str | None, model: str, prompt_tokens: int, completion_tokens: int) -> None:
        key = agent_name or "default"
        if key not in self._usage_stats:
            self._usage_stats[key] = {"prompt_tokens": 0, "completion_tokens": 0, "calls": 0, "cost": 0.0}
    ...
    def get_usage_stats(self) -> dict[str, dict]:
        """Return cumulative token usage per agent."""
    ...
    async def complete(
        ...
        log.error(
            "llm_all_failed",
            extra={"models": models_to_try, "agent": agent_name, "error": str(last_error)},
        )
        return LLMResponse(
    ...
    async def _complete_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                self._track_usage(agent_name, actual_model, prompt_tokens, completion_tokens)

                log_event(log, "llm_call", model=actual_model, agent=agent_name,
                          prompt_tokens=prompt_tokens, completion_tokens=completion_tokens,
                          duration_ms=round(elapsed * 1000), tool_calls=len(tool_calls))
                ...
                if is_retryable and attempt < max_retries:
                    ...
                    log.warning(
                        "llm_retry",
                        extra={"model": model, "agent": agent_name,
    ...
    async def stream(
        ...
        try:
            ...
            log_event(log, "llm_stream", model=model, agent=agent_name,
